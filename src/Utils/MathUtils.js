...
export default class MathUtils {
    ...
    static average(...values): number | null {
        if (values.length === 0) {
            return null;
        }
        return values.reduce((sum: number, value: number) => sum + value) / values.length;
    }
    ...
    static round(n: number | string, count: number = 0): number {
        const rank = 10 ** count;
